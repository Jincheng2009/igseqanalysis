...
metafile <- "C:/Java/data/sarav8tile/metadata.json"
datapath <- dirname(metafile)
metadata <- fromJSON(file=metafile)

remove.idx <- which(sapply(metadata, function(x) x$sample=="S5"))
remove.idx <- remove.idx[order(remove.idx, decreasing=T)]
for(index in remove.idx) {
    metadata[[index]] <- NULL
}
